 ...
 * its `customType: "advisor"` origin. The second sentence covers only a late
...
const ADVISOR_PRIMARY_CONTEXT =
  "Messages wrapped in <advisory> are generated by a separate AI advisor watching this session and trying to help; they are not authored by the user. When an advisory arrives after a completed assistant response and causes another response, the newer response may scroll the preceding response out of view; it should stand on its own without assuming the preceding response was read.";
